...
fn criterion_benchmark(c: &mut Criterion) {
    c.bench_function("parse query", |b| {
        b.iter(|| {
            hl7_parser::query::parse_location_query(black_box("MSH.1.2.3"))
                .expect("Can parse location query")
        })
    });

    c.bench_function("query message", |b| {
        let message = hl7_parser::parse_message_with_lenient_newlines(include_str!(
            "../test_assets/sample_adt_a08.hl7"
        ))
